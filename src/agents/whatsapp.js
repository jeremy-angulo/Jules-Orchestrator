...
export async function runWhatsAppAgent(project, _testHooks = {}) {
  while (true) {
    if (_testHooks.onLoopStart) await _testHooks.onLoopStart();
    try {
      if (isProjectLocked(project.id)) {
        await sleep(_testHooks.sleepTime || 30000);
        continue;
      }
      ...
      await sleep(_testHooks.sleepTime || 30000);
       ...
       decrementTasks(project.id);
       await sleep(_testHooks.sleepTime || 60000);
    }
    if (_testHooks.onLoopEnd) await _testHooks.onLoopEnd();
  }
}
